...
const LOG_LEVEL_ENV_VAR: &str = "BITBUCKET_DC_MCP_LOG_LEVEL";
const TRANSPORT_ENV_VAR: &str = "BITBUCKET_DC_MCP_TRANSPORT";
...
/// `BITBUCKET_DC_MCP_LOG_LEVEL`, defaulting to `"info"` when unset.
...
pub fn init_logging(otel_layer: Option<BoxedLayer>) {
    ...
    // protocol traffic, so logs go there instead — the conventional log
    // destination for a server process.
