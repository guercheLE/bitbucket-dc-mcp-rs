...
const ENV_PREFIX: &str = "BITBUCKET_DC_MCP";
...
/// (`./bitbucket-dc-mcp.config.yml`) -> home file
/// (`~/.bitbucket-dc-mcp/config.yml`) -> system file
/// (`/etc/bitbucket-dc-mcp/config.yml`) -> install-dir file -> built-in
...
pub fn load_config(cli_flags: Map<String, Value>) -> Result<Config, McpifyError> {
    ...
    let layers = [
        ...
        read_yaml_if_exists(&PathBuf::from("/etc/bitbucket-dc-mcp/config.yml")),
